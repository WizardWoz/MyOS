...
;7c00 ~ 7e00：这段地址固定留给MBR (BOOT)，在硬件平台上电时，BIOS自动从存储介质将Boot加载到0x7C00，Boot程序执行结束该段地址可作他用
...
SectorNumOfRootDirStart equ 25  ;根目录起始扇区号25=保留扇区数1+FAT表扇区数12*FAT表份数2，前0~24号扇区均被占用
SectorNumOfFAT1Start equ 1  ;FAT1表的起始扇区号为1（因为FAT表1前只有一个保留扇区（引导扇区）且扇区号为0）
SectorBalance equ 23    ;因为FAT[0]与FAT[1]簇为无效簇，所以将根目录起始扇区号-2，则间接将数据区起始扇区号-2
...
BPB_SecPerClus db 8         ;每个簇占用扇区数，取值必须是2的幂次（从2的0次方开始）
...
;     ret                     ;因为没有涉及call指令调用，无需使用leave指令，ret指令弹出栈顶push IP
    ...
    ; 借助栈地址空间创建16B硬盘地址包结构
    ; 0x00(1B)：硬盘地址包大小（16B：0x10/24B：0x18）
    ; 0x01(1B)：保留值（0）
    ; 0x02(2B)：传输的扇区数
    ; 0x04(4B)：传输缓存区地址（段基地址:偏移量）；16B硬盘地址包为32bit位宽，24B硬盘地址包为FFFF:FFFF
    ; 0x08(8B)：LBA型扇区起始号
    ; 0x10(8B)：24B硬盘地址包的64位传输缓存区地址
    push dword 00h
    push dword eax
    push word es
    push word bx
    push word cx
    push word 10h
;=======int 13h,AH=42h：LBA型扇区读取操作
;=======DL=驱动器号（如果操作的是硬盘驱动器，bit 7必须被置位，即硬盘编号从0x80开始）
;=======DS:SI：指向硬盘地址包结构的指针；BIOS中断程序通过这个结构来对磁盘进行扇区寻址
    mov ah,42h
    mov dl,80h
    mov si,sp
    int 13h
