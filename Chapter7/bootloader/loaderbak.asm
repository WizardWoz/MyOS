...
; 段寄存器：
; 1.为减少转换时间与编码的复杂性，处理器为保护模式下的CS、SS、DS、ES、FS、GS段寄存器加入了缓存区域（段寄存器不可见区域），记录段描述符基地址、限长、属性等信息
...
; 2.代码段描述符（GDT、LDT）：S标志位=1，Type第43位=1
...
; GDT：可供所有特权级访问的全局代码段（如操作系统内核）
; LDT：供特定任务/应用程序的代码段，提供隔离

; 3.数据段描述符（GDT、LDT）：S标志位=1，Type第43位=0
...
; GDT：可供所有特权级访问的全局数据段（如操作系统的数据结构、共享内存）
; LDT：用于特定任务或应用程序的数据段（如某个程序的堆栈、堆或私有数据）

; 4.TSS段描述符（GDT）：S标志位=0，Type=10B1（B=0为非忙状态，B=1为忙状态运行/挂起），D/B=0
...
; 而任务的执行现场将通过手动方式保存于栈/PCB（进程控制块）；为大多数操作系统采用
; GDT：总是位于GDT。每个任务状态段（TSS）代表一个独特的任务或进程

; 5.LDT段描述符（GDT）：用于记录LDT表的位置、长度、访问权限等信息，位功能与TSS段描述符一样
; LDT可在各程序/任务间起到隔离作用；也可为每个任务创建独立页表结构来隔离任务
; GDT：总是位于GDT。一个系统可以有多个LDT（每个任务或一组任务一个），GDT必须包含所有活动LDT的描述符

; 6.调用门描述符（GDT、LDT）：让不同特权级间的程序实现受控切换，通常用于受特权级保护的操作系统/程序；借助CALL指令穿过调用门可访问更高特权级的代码段
; 如果目标代码段是非一致性的，则栈切换会发生（一致性代码段不会改变CS寄存器的CPL值）；特权级切换必定会伴随栈切换，而且栈特权级=CPL特权级
; (1)第0~15位、第48~63位：32位段内偏移量（指定程序入口地址）
; (2)第16~31位：16位指定段选择子，48位远跳转地址=段选择子:段内偏移，为处理器提供中断/异常处理程序入口地址
; (3)第32~36位：参数个数
; (4)第37~39位：保留位=0
; (5)第40~43位Type：等于1100，指定段描述符类型，对S标志位作进一步解释
; (6)第44位S：S=0，属于系统段描述符
; (7)第45~46位DPL：段描述符特权级（0~3，数值越小级别越高）
; (8)第47位P：P=0段不在内存，P=1段已在内存；如果段寄存器加载一个不在内存的段描述符将会触发#NP异常（修改时使用LOCK前缀锁总线）
; GDT：该调用门需要全局可访问
; LDT：该调用门特定于某个任务
...
; (13)第30位CD：控制系统内存缓存机制（影响PDE、PTE页表项的PWT、PCD标志位）
; (14)第31位PG：使能分页管理机制
; CD=0、NW=0：标准缓冲模式，最高效的缓存策略
; CD=0、NW=1：无效设置，触发错误码为0的#GP异常
; CD=1、NW=0：处理器无法缓存数据，需要保持内存的一致性
; CD=1、NW=1：处理器无需保持内存与缓存的一致性

; 2.CR1：第0~31位保留=0

; 3.CR2：第0~31位引起#PF异常的线性地址，不会对页错误的线性地址进行检测

; 4.CR3（PDBR）：记录页目录的物理基地址属性，不会对页目录基地址进行检测
; (1)第0~2位：保留=0
; (2)第3位PWT：页级写穿标志位
; (3)第4位PCD：页级禁止缓存标志位
; (4)第5~11位：保留=0
...
; 保护模式的页管理机制
; 1.32位分页模式（CR0.PG=1）：保护模式的基础页管理模式，线性地址位宽与物理地址位宽同为32位，并未扩展物理地址寻址能力
; (1)使用二级页表映射：10bit页目录项索引，10bit页表项索引，12bit页内偏移量（4KB物理页）
; (2)CR3寄存器（4B）：
; <1>第0~2位、第5~11位：无效
; <2>第3位PWT：页级写穿标志位；PWT=0回写，PWT=1写穿（CR0.CD=1会忽略）
; <3>第4位PCD：页级禁止缓存标志位；PCD=0页可以缓存，PCD=1页不能缓存（CR0.CD=1会忽略）
; <4>第12~31位：页目录表PDT的物理基地址
; (3)页目录表项PDE（4B）：
; <1>第0位P：物理页存在标志位；P=0不存在内存（访问会触发#PF异常），P=1存在内存
; <2>第1位R/W：读写操作标志位；R/W=0只读页，R/W=1可读写页
; <3>第2位U/S：访问模式标志位；U/S=0超级模式（只允许特权级0、1、2的程序访问），U/S=1用户模式（允许任何特权级的程序访问）
; <4>第3位PWT：页级写穿标志位；PWT=0回写，PWT=1写穿（CR0.CD=1会忽略）
; <5>第4位PCD：页级禁止缓存标志位；PCD=0页可以缓存，PCD=1页不能缓存（CR0.CD=1会忽略）
; <6>第5位A：访问标志位；A=0未访问，A=1已访问（处理器只能置位无法复位）
; <7>第6位、第8~11位：无效位
; <8>第7位：为0
; <9>第12~31位：页表PT的物理基地址
; (4)页表项PTE（4B）：
; <1>第0位P：物理页存在标志位；P=0不存在内存（访问会触发#PF异常），P=1存在内存
; <2>第1位R/W：读写操作标志位；R/W=0只读页，R/W=1可读写页
; <3>第2位U/S：访问模式标志位；U/S=0超级模式（只允许特权级0、1、2的程序访问），U/S=1用户模式（允许任何特权级的程序访问）
...
; <5>第4位PCD：页级禁止缓存标志位；PCD=0页可以缓存，PCD=1页不能缓存（CR0.CD=1会忽略）
; <6>第5位A：访问标志位；A=0未访问，A=1已访问（处理器只能置位无法复位）
; <7>第6位D：脏页标志位；D=0干净，D=1已脏（处理器只能置位无法复位）
; <8>第7位PAT：页面属性标志位；扩展了PCD位和PWT位的功能，与MTRR寄存器协同工作指定访问物理内存的类型
; <9>第8位G：全局页标志位；G=0局部页面，G=1全局页面
; <10>第9~11位：无效位
; <11>第12~31位：4KB物理页的物理基地址
; <12>PS标志位：32位模式下无此标志位，用于指明页表项索引的是物理页还是次级页表
; (5)IA32_PAT寄存器：设置页属性PAT的内存类型，位于MSR寄存器组的277h地址处；包含8个属性区域（PA0~PA7），
; 每个属性区域的低3位用于指明内存类型，高5位保留使用（必须设置为0）
; <1>通过wrmsr指令可修改PAT表的内存类型，默认内存类型如下
; PAT=0，PCD=0，PWT=0：PAT0，内存类型WB，整个读写操作均在缓存行中进行。写操作将延迟更新到内存，从而减少内存访问次数
; PAT=0，PCD=0，PWT=1：PAT1，内存类型WT，读写均被缓存，读操作先访问缓存行；写操作将同时写入到缓存行和内存
; PAT=0，PCD=1，PWT=0：PAT2，内存类型UC-，与UC功能相同，但可在MTRR寄存器中使用WC覆盖
; PAT=0，PCD=1，PWT=1：PAT3，内存类型UC，内存不可缓存，对设备内存I/O映射非常有用，在物理内存中将导致执行效率降低

; 2.PSE模式（CR4.PSE=1）：32位分页模式的扩展，允许处理器在保护模式下使用4MB物理页
; (1)CPUID.01h:EDX[3].PSE位检测是否支持PSE-32功能；CPUID.01h:EDX[17].PSE-36位检测是否支持PSE-36功能
; PSE-36模式扩展了处理器的物理地址寻址能力（32位——>36位/40位），通过CPUID.80000008h:EAX[7:0]获取处理器最高物理可寻址位数MAXPHYADDR，
; 同时还要兼顾BIOS中断服务程序int 15h，AX=E820h提供的物理内存信息
; 桌面级处理器普遍支持36位物理地址位宽，服务级处理器普遍支持40位物理地址位宽，暂时没有处理器支持52位物理地址位宽
; (2)只使用一级页表映射，10bit页目录项索引，22bit页内偏移量（4MB物理页）
; (3)CR3寄存器（4B）：
; <1>第0~2位、第5~11位：无效
; <2>第3位PWT：页级写穿标志位；PWT=0回写，PWT=1写穿（CR0.CD=1会忽略）
; <3>第4位PCD：页级禁止缓存标志位；PCD=0页可以缓存，PCD=1页不能缓存（CR0.CD=1会忽略）
; <4>第12~31位：页目录表PDT的物理基地址
; (4)4MB物理页的页目录表项PDE（4B）：
; <1>第0位P：物理页存在标志位；P=0不存在内存（访问会触发#PF异常），P=1存在内存
; <2>第1位R/W：读写操作标志位；R/W=0只读页，R/W=1可读写页
; <3>第2位U/S：访问模式标志位；U/S=0超级模式（只允许特权级0、1、2的程序访问），U/S=1用户模式（允许任何特权级的程序访问）
; <4>第3位PWT：页级写穿标志位；PWT=0回写，PWT=1写穿（CR0.CD=1会忽略）
...
; <9>第8位G：全局页标志位；G=0局部页面，G=1全局页面
; <10>第9~11位：无效位
; <11>第12位PAT：页面属性标志位；扩展了PCD位和PWT位的功能，与MTRR寄存器协同工作指定访问物理内存的类型
; <12>第13~20位：4MB物理页的基地址
; <13>第21位：保留位=0
; <14>第22~31位：4MB物理页的基地址

; 3.PAE模式（CR4.PAE=1）：CPUID.01h:EDX[6].PAE位检测处理器是否支持PAE模式；将32位线性地址从原有的二级页表映射扩展为三级，
; 每个页表项从原来的32位（4B）增长至64位（8B）
; (1)在页目录表PDT和页表PT的基础上，又引入了页目录指针表PDPT，只有4个表项，每个表项占8B共32B，相比PSE模式进一步扩展物理地址寻址能力（36/40bit——>52bit）
; (2)使用三级页表映射（PDE.PS=0）：2bit页目录指针表项PDPT索引，9bit页目录项PDT索引，9bit页表项PT索引，12bit页内偏移量
; 或者使用二级页表映射（PDE.PS=1）：2bit页目录指针表项PDPT索引，9bit页目录项PDT索引，21bit页内偏移量
; (3)CR3寄存器（8B，但只使用低4B）：
; <1>第0~4位、第32~63位：无效位
; <2>第5~31位：页目录指针表PDPT的物理基地址，必须按32B边界对齐，所以PDPT表只能保存于低4GB内存中且起始地址按32B边界对齐
; (4)页目录指针表项PDPTE（8B）：
; <1>第0位P：物理页存在标志位；P=0不存在内存（访问会触发#PF异常），P=1存在内存
; <2>第1~2位、第5~8位、第M~63位：保留=0（M小于等于63）
; <3>第3位PWT：页级写穿标志位；PWT=0回写，PWT=1写穿（CR0.CD=1会忽略）
; <4>第4位PCD：页级禁止缓存标志位；PCD=0页可以缓存，PCD=1页不能缓存（CR0.CD=1会忽略）
; <5>第9~11位：无效位
; <6>第12~M-1位：页目录PDT的物理基地址（M小于等于63）
; (5)2MB物理页的页目录表项PDE（8B）：
; <1>第0位P：物理页存在标志位；P=0不存在内存（访问会触发#PF异常），P=1存在内存
; <2>第1位R/W：读写操作标志位；R/W=0只读页，R/W=1可读写页
; <3>第2位U/S：访问模式标志位；U/S=0超级模式（只允许特权级0、1、2的程序访问），U/S=1用户模式（允许任何特权级的程序访问）
; <4>第3位PWT：页级写穿标志位；PWT=0回写，PWT=1写穿（CR0.CD=1会忽略）
...
; <11>第12位PAT：页面属性标志位；扩展了PCD位和PWT位的功能，与MTRR寄存器协同工作指定访问物理内存的类型
; <12>第13~20位：保留位
; <13>第21~M-1位：2MB物理页的基地址（M小于等于62）
; <14>第M~62位：保留=0（M小于等于62）
; <15>第63位XD：禁止执行标志位；禁止处理器从物理页中获取指令（CPUID.80000001h:EDX[20].XD位可查询是否支持）
; 在支持XD功能的前提下，置位IA32_EFER.NXE=1开启XD功能，否则XD位保留且必须为0
; (6)4KB物理页的页目录表项PDE（8B）：
; <1>第0位P：物理页存在标志位；P=0不存在内存（访问会触发#PF异常），P=1存在内存
; <2>第1位R/W：读写操作标志位；R/W=0只读页，R/W=1可读写页
; <3>第2位U/S：访问模式标志位；U/S=0超级模式（只允许特权级0、1、2的程序访问），U/S=1用户模式（允许任何特权级的程序访问）
; <4>第3位PWT：页级写穿标志位；PWT=0回写，PWT=1写穿（CR0.CD=1会忽略）
...
; <8>第7位：为0
; <9>第8~11位：无效位
; <10>第12~M-1位：页表PT的物理基地址（M小于等于62）
; <11>第M~62位：保留=0（M小于等于62）
; <12>第63位XD：禁止执行标志位；禁止处理器从物理页中获取指令（CPUID.80000001h:EDX[20].XD位可查询是否支持）
; 在支持XD功能的前提下，置位IA32_EFER.NXE=1开启XD功能，否则XD位保留且必须为0
; (7)4KB物理页的页表项PTE（8B）：
; <1>第0位P：物理页存在标志位；P=0不存在内存（访问会触发#PF异常），P=1存在内存
; <2>第1位R/W：读写操作标志位；R/W=0只读页，R/W=1可读写页
...
; <9>第8位G：全局页标志位；G=0局部页面，G=1全局页面
; <10>第9~11位：无效位
; <11>第12~M-1位：4KB物理页的物理基地址（M小于等于62）
; <12>第M~62位：保留=0（M小于等于62）
; <13>第63位XD：禁止执行标志位；禁止处理器从物理页中获取指令（CPUID.80000001h:EDX[20].XD位可查询是否支持）
; 在支持XD功能的前提下，置位IA32_EFER.NXE=1开启XD功能，否则XD位保留且必须为0
...
;=======3.BIOS int15h的AX=2400子功能禁用A20、AX=2401开启A20、AX=2403查询A20状态；4.读0xEE端口开启A20、写0xEE端口禁止A20
    ...
    mov eax,cr0
    and al,11111110b    ;CR0寄存器的第0位被重新置为0，关闭保护模式，重新进入实模式
    mov cr0,eax         ;如果此时重新对FS段寄存器赋值
    sti
...
;=======获取物理地址空间信息（由一个20B结构体数组构成，包括可用物理内存地址空间、设备寄存器地址空间、内存空洞）
;0x00~0x07（8B）：起始地址；0x08~0x0F（8B）：长度（以字节为单位）；0x10~0x13（4B）：内存类型
;内存类型：0x01：可用物理内存；0x02：保留/无效值（ROM/设备内存）；0x03：ACPI的回收内存；0x04：ACPINVS内存；其它值：未定义，保留使用
Label_Get_Mem_Struct:
    ;int 15h子功能号AH=E8H：获取物理地址空间信息，并将其保存在ES:DI=0x07E00地址处
    ;参数：EAX=0x0000E820；EDX=0x534D4150（字符串"SMAP"）；EBX=0x00000000（起始映射结构体，其它值为后续映射结构体）
    ;ECX=预设返回结果的缓存区结构体长度，以字节为单位（大于等于20B）；ES:DI=保存返回结果的缓存地址
    ;返回值：CF=0，说明操作成功；EAX=0x534D4150（字符串"SMAP"）；ES:DI=保存返回结果的缓存地址；
    ;EBX=0x00000000（为0表明检测结束，其它值为后续映射结构体）；ECX=实际返回结果的缓存区结构体长度，以字节为单位（大于等于20B）
    ;CF=1，说明操作失败；AH=错误码（0x80：无效命令；0x86：不支持此功能）
    mov eax,0x0E820
    mov ecx,20
...
; 保护模式的中断/异常处理机制
; 1.中断门描述符（IDT）：位图结构上与调用门描述符非常相似，包含一个远跳转地址（段选择子&段内偏移），为处理器提供中断/异常处理程序的入口地址
; (1)第32~36位：保留位=0
; (2)第40~43位Type：Type=D110，D标志位指示门描述符的数据位宽
; IDT：总是位于IDT。其余位功能与调用门描述符相同

; 2.陷阱门描述符（IDT）：位图结构上与调用门描述符非常相似，包含一个远跳转地址（段选择子&段内偏移），为处理器提供中断/异常处理程序的入口地址
; (1)第32~36位：保留位=0
; (2)第40~43位Type：Type=D111，D标志位指示门描述符的数据位宽
; 其余位功能与调用门描述符相同
; 中断门描述符与陷阱门描述符的不同之处体现在处理器对IF标志位（EFLAGS标志寄存器）的操作上：
; 当处理器通过中断门描述符执行中断/异常处理程序时，处理器会复位IF标志位IF=0；处理程序执行完毕iret指令还原保存在
; 栈中的EFLAGS寄存器（包括IF标志位）；当处理器通过陷阱门描述符执行中断/异常处理程序时，将不会对IF标志位进行操作
; IDT：总是位于IDT。其余位功能与调用门描述符相同

; 3.任务门描述符（GDT、LDT、IDT）：在任务切换过程中提供一些间接保护措施。位结构最简单（Linux内核从未使用过，且IA-32e模式也已取消）
; (1)第0~15位、第32~39位、第48~63位：保留位=0
; (2)第16~31位：TSS段选择子（RPL区域不会被使用）
; (3)第40~43位Type：Type=0101
; 其余位功能与调用门描述符相同
; (4)当程序/任务借助call/jmp指令穿过任务门访问目标程序时，CS寄存器的CPL值与任务门描述符选择子的RPL值在数值上，
; 必须小于/等于任务门描述符的DPL值，且目标TSS描述符的DPL不起作用
; GDT、LDT：可由call或jmp指令切换到另一个任务
; IDT：可用于在中断/异常时切换任务，通常用于在专用的错误处理任务中处理关键错误

;=======为IDT开辟内存空间，因为切换至保护模式前Loader.bin已使用cli指令关闭中断，进而不必完整初始化IDT
;=======只须有相应的结构体即可（目前为空）；若能保证模式切换过程中不产生异常，没有IDT也可以
IDT:
    times 0x50 dq 0
IDT_END:
