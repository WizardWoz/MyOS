...
/*extern是C语言中的一个存储类说明符（storage class specifier）它的主要作用是声明一个变量或函数，
  表明这个变量或函数是在其他地方定义的（可能是在同一个文件的后面，或者更常见的是在另一个 C 文件中）
*/

extern struct desc_struct GDT_Table[];  //外部文件可引用的全局描述符表GDT结构
extern struct gate_struct IDT_Table[];  //中断描述符表IDT结构
