...
void init_memory()
{
    ...
    // struct Memory_E820_Formate *p = NULL;    //P122
    struct E820 *p = NULL;
    ...
    {
        // 获取总可用物理内存空间
        //  P122把64位的线性地址拆分为两段显示：%#010x显示'0x'和高32位；%08x显示低32位
    ...
    color_printk(ORANGE, BLACK, "OS Can Used Total 2M PAGES:%#010x=%010d\n", TotalMem, TotalMem); // 在屏幕上打印系统可用的2MB物理页数

    // 初始化全局内存管理结构体变量memory_management_struct包含的页映射位图
    ...
    // 初始化全局内存管理结构体变量memory_management_struct包含的物理页信息
    ...
    // 初始化全局内存管理结构体变量memory_management_struct包含的物理空间结构信息
    ...
    // 初始化全局内存管理结构体变量memory_management_struct包含的物理内存段结构数组E820
    ...
    //在屏幕上打印内核程序起始代码段地址、内核程序结束代码段地址、内核程序结束数据段地址、内核程序结束地址、内存页管理结构的结尾地址
    ...
    i = Virt_To_Phy(memory_management_struct.end_of_struct) >> PAGE_2M_SHIFT;//得到系统内核、内存管理单元结构的所有物理页数
    //系统内核、内存管理单元结构所占用的物理页struct Page页属性初始化
    for (j = 0; j <= i; j++)
    {
