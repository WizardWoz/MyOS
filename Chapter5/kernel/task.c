...
unsigned long init(unsigned long arg)
{
	...
	current->thread->rip=(unsigned long)ret_system_call;	//init进程的返回地址
	current->thread->rsp=(unsigned long)current+STACK_SIZE-sizeof(struct pt_regs);//init进程的栈指针
	regs=(struct pt_regs *)current->thread->rsp;	//init进程的struct pt_regs执行现场结构体
	__asm__ __volatile__(
	//指令部分：（参考宏函数switch_to的思路，调用execve系统调用API的处理函数do_execve）
	//movq %1,%%rsp：RSP=将内存中保存的当前进程的内核层栈指针
		"movq %1,%%rsp	\n\t"
	//pushq %2：将当前进程的内核层代码指针（程序的返回地址）入栈
		"pushq %2	\n\t"
	//jmp do_execve：采用cdecl调用约定来调用C语言函数、系统调用API：do_execve，参数RDI=struct pt_regs *regs指针
		"jmp do_execve	\n\t"
	//输出部分：无
		:
	//输入部分：所有指令执行前，RDI=struct pt_regs *regs指针；当前进程的内核层栈指针、内核层代码指针均保存在内存
		:"D"(regs),"m"(current->thread->rsp),"m"(current->thread->rip)
	//损坏描述：指令的执行已经对内存产生了影响，应用memory声明
		:"memory"
	);
	return 1;
}
...
unsigned long do_fork(struct pt_regs *regs,unsigned long clone_flags,unsigned long stack_start,unsigned long stack_size)
{
	...
	*tsk=*current;
	...
	if (!(tsk->flags&PF_KTHREAD))
	{
		...
		//如果目的进程不是内核级线程，则将进程执行入口地址设置在ret_system_call
...
/*
  函数：设置struct pt_regs结构体成员变量的值来构建应用程序执行环境
  参数：
  1.struct pt_regs *regs：指向当前进程的执行现场struct pt_regs的指针
  返回值：unsigned long，目前暂时定为1

  当do_execve函数返回时，处理器跳转至ret_system_call模块处执行，将struct pt_regs结构体各成员变量还原到对应寄存器
*/
unsigned long do_execve(struct pt_regs *regs)
{
	//选用regs->rdx、regs->rcx是与sysexit指令需求对应
	regs->rdx=0x800000;		//regs->rdx：保存应用层程序的入口地址（只要是未使用的线性地址空间均可）
	regs->rcx=0xa00000;		//regs->rcx：保存应用层程序的栈顶地址（只要是未使用的线性地址空间均可）
	regs->rax=1;
	regs->ds=0;
	regs->es=0;
...
int kernel_thread(unsigned long (*fn)(unsigned long),unsigned long arg,unsigned long flags)
{
    ...
    return do_fork(&regs,flags,0,0);	//返回do_fork函数的调用值
}
