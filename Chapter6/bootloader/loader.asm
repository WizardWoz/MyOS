...
/*
  保护模式的页管理机制
  1.CR3控制寄存器（页目录基地址寄存器PDBR）：详情见上面笔记

  2.32位分页模式：保护模式的基础页管理模式，线性地址位宽与物理地址位宽同为32位，并未扩展物理地址的寻址能力
  (1)页目录表项PDE（4B）：
  <1>第0位P：物理页存在标志位；P=0不存在内存，P=1存在内存
  <2>第1位R/W：读写操作标志位；R/W=0只读页，R/W=1可读写页
  <3>第2位U/S：访问模式标志位；U/S=0超级模式，U/S=1用户模式
  <4>第3位PWT：页级写穿标志位；PWT=0回写，PWT=1写穿
  <5>第4位PCD：页级禁止缓存标志位；PCD=0页可以缓存，PCD=1页不能缓存
  <6>第5位A：访问标志位；A=0未访问，A=1已访问
  <7>第6位、第8~11位：无效位
  <8>第7位：为0
  <9>第12~31位：页表的物理基地址PT

  (2)页表项PTE（4B）：
  <1>第0位P：物理页存在标志位；P=0不存在内存（触发#PF异常），P=1存在内存
  <2>第1位R/W：读写操作标志位；R/W=0只读页，R/W=1可读写页
  <3>第2位U/S：访问模式标志位；U/S=0超级模式（只允许特权级0、1、2的程序访问），U/S=1用户模式（允许任何特权级的程序访问）
  <4>第3位PWT：页级写穿标志位；PWT=0回写，PWT=1写穿
  <5>第4位PCD：页级禁止缓存标志位；PCD=0页可以缓存，PCD=1页不能缓存
  <6>第5位A：访问标志位；A=0未访问，A=1已访问
  <7>第6位D：脏页标志位；D=0干净，D=1已脏
  <8>第7位PAT：页面属性标志位；扩展了PCD位和PWT位的功能，与MTRR寄存器协同工作指定访问物理内存的类型
  <9>第8位G：全局页标志位；G=0局部页面，G=1全局页面
  <10>第9~11位：无效位
  <11>PS标志位：32位模式下无此标志位，用于指明页表项索引的是物理页还是次级页表

  (3)IA32_PAT寄存器：设置页属性PAT的内存类型，位于MSR寄存器组的277h地址处；包含8个属性区域（PA0~PA7），
