...
/*
  函数：向MSR寄存器组的某个地址处的子寄存器写入数据
  参数：
  1.unsigned long address：MSR寄存器组的子寄存器的内部地址
  2.unsigned long value：要写入的数据值
*/
inline void wrmsr(unsigned long address,unsigned long value)
{
	__asm__ __volatile__(
	//指令部分：
	//wrmsr：向ECX存放的地址处的MSR寄存器组的子寄存器写入EDX:EAX数值
		"wrmsr	\n\t"
	//输出部分：无
		:
	//输入部分：在所有指令执行前，EDX=64位变量value的高32位；EAX=64位变量value的低32位；ECX=子寄存器内部地址
		:"d"(value>>32),"a"(value&0xFFFFFFFF),"c"(address)
	//损坏描述：指令执行过程中可能会影响到内存，用memory声明
		:"memory");
}
